...
<a href="/chizhovy2/otzyvy/">Истории учеников</a>
...
<p style="margin-top:26px"><a class="btn btn-ghost" href="/chizhovy2/otzyvy/">Читать истории целиком</a></p>
</div></section>
...
<p style="margin-top:26px"><a class="btn btn-wine" href="/chizhovy2/sessiya/">Записаться на&nbsp;собеседование вдвоём</a></p>
</div></section>
""")
...
<p>На&nbsp;рынке принято прятать истоки: метод подаётся как озарение основателя, а&nbsp;источники остаются за&nbsp;кадром. Нам такой театр не&nbsp;нужен. Взрослому человеку важно понимать, на&nbsp;что он&nbsp;опирается, и&nbsp;у&nbsp;него есть право проверить каждый корень нашей работы.</p>
<p>К&nbsp;тому&nbsp;же честность про истоки сама по&nbsp;себе часть метода. Школа начинается с&nbsp;точки&nbsp;А: признать, где ты&nbsp;на&nbsp;самом деле. Мы&nbsp;делаем то&nbsp;же самое как школа: показываем, откуда выросли.</p>
</div></section>
...
<div class="card"><span class="bignum">03</span>{icon('flame','var(--sand)')}<h3>est: тренинги погружения</h3><p>Сан-Франциско, 1971&nbsp;год, Вернер Эрхард. Два уикенда, после которых жизнь делилась на&nbsp;до&nbsp;и&nbsp;после. Корень жанра, в&nbsp;котором работает школа.</p><p style="margin-top:12px"><a href="/chizhovy2/istoki/est-transformaciya/">Разобрать</a></p></div>
<div class="card"><span class="bignum">04</span>{icon('sunrise')}<h3>Невилл Годдард: состояние</h3><p>«Реальность откликается на&nbsp;состояние, а&nbsp;не&nbsp;на&nbsp;просьбу». Основа практики намерения, которую ученики ведут девяносто дней Марафона.</p><p style="margin-top:12px"><a href="/chizhovy2/istoki/goddard/">Разобрать</a></p></div>
<div class="card"><span class="bignum">05</span>{icon('book','var(--sage-deep)')}<h3>Наука: почему это работает</h3><p>ЛеДу, Гоулман, Болте Тейлор, Голвитцер, Либерман. Каждый термин истоков стоит у&nbsp;нас на&nbsp;научной подпорке.</p><p style="margin-top:12px"><a href="/chizhovy2/istoki/nauka/">Разобрать</a></p></div>
...
<p>Главная ставка психодрамы: о&nbsp;проблеме бесполезно рассказывать, в&nbsp;неё нужно вернуться. Участники группы становятся героями твоей истории, пространство зала превращается в&nbsp;ту&nbsp;кухню или тот кабинет, и&nbsp;сцена оживает.</p>
<div class="grid3" style="margin-top:26px">
...
<div class="card white" style="margin:20px 0 12px">{icon('loop')}<h3>Маятники</h3><p>Структуры, которые кормятся твоей реакцией: скандал, лента новостей, чужая паника, офисная война. Дёрнулся, значит отдал энергию. У&nbsp;Зеланда это образ; в&nbsp;зале он&nbsp;становится навыком: заметить крючок и&nbsp;не&nbsp;схватиться.</p></div>
...
<div class="card white">{icon('lens','var(--sand)')}<h3>Зеркало мира</h3><p>Мир откликается на&nbsp;состояние, а&nbsp;не&nbsp;на&nbsp;слова. Пока внутри страх, снаружи собираются поводы бояться. Ученики после тренинга говорят коротко: мир зеркалит.</p></div>
...
<section class="dark"><div class="narrow">
<h2>Что мы&nbsp;взяли и&nbsp;что переработали</h2>
<p><b style="color:#D08A5F">Взяли:</b> рабочий язык. На&nbsp;Марафоне трансерфинг входит в&nbsp;список чтения, а&nbsp;его термины живут в&nbsp;ежедневной практике: утром намерение, вечером разбор, где катался на&nbsp;маятниках и&nbsp;где удержал состояние.</p>
...
<p>Люди сидели в&nbsp;зале по&nbsp;шестнадцать часов, сдавали часы на&nbsp;входе и&nbsp;держали слово не&nbsp;вставать до&nbsp;перерыва. Ведущий разбирал их&nbsp;истории при всех, без анестезии. Звучит жёстко, и&nbsp;это было жёстко. Но&nbsp;у&nbsp;формата оказалась настоящая сила: за&nbsp;два уикенда защита психики, которую час терапии даже не&nbsp;царапает, снималась, и&nbsp;человек впервые видел свою жизнь без привычных оправданий.</p>
...
<div class="card"><h3>Понять и&nbsp;пережить</h3><p>est первым развёл эти вещи: понимание живёт в&nbsp;голове и&nbsp;ничего не&nbsp;меняет. Меняет пережитый опыт. Вся индустрия погружений выросла из&nbsp;этого различия.</p></div>
...
<p><b style="color:#D08A5F">Переработали:</b> почти всё остальное. est работал залами по&nbsp;двести пятьдесят человек и&nbsp;провокацией; мы&nbsp;выбрали противоположный масштаб. Группы 10-20 человек, бережная глубина вместо давления, психодрама вместо лекции со&nbsp;сцены и&nbsp;три месяца сопровождения после, чтобы результат не&nbsp;выветрился к&nbsp;понедельнику.</p>
</div></section>
...
<p>Реальность откликается на&nbsp;состояние, а&nbsp;не&nbsp;на&nbsp;просьбу. Просить и&nbsp;ждать бесполезно, пока внутри ты&nbsp;остаёшься человеком, у&nbsp;которого «пока не&nbsp;получилось»: из&nbsp;такого состояния рождаются те&nbsp;же действия и&nbsp;те&nbsp;же события, что вчера. Годдард предлагал обратный ход: сначала прожить состояние человека, у&nbsp;которого уже есть, и&nbsp;дать этому состоянию вести решения.</p>
<p>Звучит смело. Но&nbsp;вспомни, как легко даётся день, когда ты&nbsp;с&nbsp;утра в&nbsp;силе, и&nbsp;как вязнет тот&nbsp;же список дел в&nbsp;день, когда внутри тяжесть. Состояние уже управляет твоими событиями, вопрос только в&nbsp;том, кто управляет состоянием.</p>
</div></section>
...
<p><b style="color:#D08A5F">Переработали:</b> рамку. У&nbsp;Годдарда мистика середины прошлого века, мы&nbsp;же показываем механизм: состояние управляет фильтрами внимания и&nbsp;качеством решений. Меняется состояние, и&nbsp;человек замечает другие возможности, делает другие шаги, получает другие события. У&nbsp;практики намерения есть и&nbsp;научный двойник: психолог Питер Голвитцер показал, что конкретно сформулированное намерение в&nbsp;разы повышает шанс дойти до&nbsp;действия.</p>
</div></section>
...
<div class="card white" style="margin-bottom:12px"><h3>Питер Голвитцер: сила намерения</h3><p>Конкретно сформулированное намерение в&nbsp;разы повышает шанс действия по&nbsp;сравнению с&nbsp;простым «надо&nbsp;бы». Наша утренняя практика намерения на&nbsp;Марафоне стоит на&nbsp;этом открытии.</p></div>
...
<p style="margin-top:26px">И&nbsp;над всем этим сто лет психодрамы Морено: групповой формат, в&nbsp;котором эти механизмы включаются не&nbsp;по&nbsp;одному, а&nbsp;разом, в&nbsp;живой сцене. Наука здесь рамка честности: мы&nbsp;берём проверенные механизмы и&nbsp;не&nbsp;обещаем чудес.</p>
</div></section>
...
<div class="in"><p class="eyebrow">Библиотека школы</p><h1>Статьи, после которых что-то щёлкает</h1>
...
<h2>Пять разделов, 26 тем</h2>
<p class="sub">Библиотека наполняется: каждая готовая статья появится здесь ссылкой. Пока темы в&nbsp;работе, начни с&nbsp;гайда, он&nbsp;собирает главное ядро метода в&nbsp;одном выпуске.</p>
