...
<p>Сильной работе нечего прятать. Мы&nbsp;открыто называем школы и&nbsp;авторов, на&nbsp;которых выросли: психодрама Морено, трансерфинг Зеланда, практика состояния Годдарда. И&nbsp;показываем, что взяли, что переработали за 16&nbsp;лет и&nbsp;почему это работает с&nbsp;точки зрения науки.</p>
...
<p>Работают с&nbsp;одним местом: там, где ты&nbsp;раз за&nbsp;разом принимаешь одно и&nbsp;то&nbsp;же решение. И&nbsp;со&nbsp;вторым вопросом, который обычно себе не&nbsp;задают: чего оно тебе стоит в&nbsp;пересчёте на&nbsp;прожитые годы.</p>
</div>
</div>
</div></section>

<section><div class="narrow">
<h2>Как выглядит одна запись целиком</h2>
<p>Возьмём самую частую: любовь надо заслужить. Ребёнок когда-то заметил, что тепло дома выдают за&nbsp;что-то: за&nbsp;пятёрку, за&nbsp;помытый пол, за&nbsp;то, что не&nbsp;мешал. Дальше человек платит всю жизнь, чем умеет: закрытыми вопросами, деньгами, готовностью выручить в&nbsp;любой час.</p>
<p>А&nbsp;дома всё идёт наоборот. Тот, кто вложился, ждёт возврата и&nbsp;незаметно для себя выставляет счёт: я&nbsp;для вас всё, а&nbsp;вы. Второй чувствует за&nbsp;подарком обязательство и&nbsp;отходит на&nbsp;шаг. Оба стараются, оба обижены, и&nbsp;каждый уверен, что дело в&nbsp;другом.</p>
<p>На&nbsp;сцене разбирают ту&nbsp;первую сделку: день, когда маленький человек решил, что его любят за&nbsp;что-то. Меняется это решение, и&nbsp;счёт закрывается сам. Близость перестаёт быть платной.</p>
</div></section>


<section><div class="narrow">
<p class="eyebrow">Почему разговоры не помогают</p>
<h2>Два рабочих слова</h2>
<p>Жертва: со&nbsp;мной это происходит. Автор, он&nbsp;же источник: я&nbsp;это создаю.</p>
<p>Пример из&nbsp;будней. Начальник третий год подкладывает чужие задачи, и&nbsp;ты&nbsp;их&nbsp;берёшь. Из&nbsp;позиции жертвы всё ясно: сел на&nbsp;шею, совести нет. У&nbsp;автора вопрос другой: что ты&nbsp;делаешь в&nbsp;ту&nbsp;секунду, когда он&nbsp;подходит, и&nbsp;что мешало все три года сказать «нет».</p>
<p>История одна и&nbsp;та&nbsp;же. Разница в&nbsp;том, что в&nbsp;первом случае сделать нельзя ничего, остаётся ждать, пока изменится он. Во&nbsp;втором появляется ход.</p>
<p>Про вину речи нет. Старая запись срабатывает раньше, чем человек успевает подумать, и&nbsp;упрекать тут не&nbsp;за&nbsp;что. Метод про&nbsp;то, чтобы это место стало видно, и&nbsp;дальше уже есть из&nbsp;чего выбирать.</p>

<h2>Три ступени: спящий, игрок, творец</h2>
<p>У&nbsp;пары «жертва и&nbsp;автор» есть середина, и&nbsp;в&nbsp;ней стоит большинство взрослых людей. Ступеней на&nbsp;самом деле три.</p>
<div class="grid3" style="margin:26px 0">
<div class="card">{icon('lens','var(--copper)')}<h3>Спящий</h3><p>Живёт в&nbsp;следствиях и&nbsp;связи не&nbsp;видит. Начальник самодур, жена холодная, рынок просел, здоровье подводит. Всё это будто погода за&nbsp;окном: случается с&nbsp;ним, а&nbsp;он&nbsp;подстраивается. Старается честно, только сделать ничего нельзя: причина каждый раз снаружи.</p></div>
<div class="card">{icon('loop','var(--sand)')}<h3>Игрок</h3><p>Связь уже почувствовал и&nbsp;взялся действовать. Меняет работу, город, партнёра, режим дня, окружение. Первые месяцы легче, а&nbsp;потом знакомое возвращается, только с&nbsp;новыми лицами и&nbsp;другим адресом. Корень остаётся прежним и&nbsp;переезжает вместе с&nbsp;ним.</p></div>
<div class="card">{icon('sunrise','var(--sage-deep)')}<h3>Творец</h3><p>Занимается причиной, и&nbsp;это тот самый автор из&nbsp;пары выше. Его интересует состояние, с&nbsp;которым он&nbsp;зашёл в&nbsp;разговор, и&nbsp;решение, что сработало раньше слов. Стоит ему сдвинуться, и&nbsp;те&nbsp;же люди отвечают иначе.</p></div>
</div>
<p>На&nbsp;средней ступени можно ходить кругами лет двадцать и&nbsp;считать это развитием.</p>
<p>Об&nbsp;этом Алексей говорит двумя фразами. Первая: куда смотришь, там и&nbsp;живёшь. Внимание и&nbsp;есть выбор места, потому что разглядываемое каждый день забирает силы и&nbsp;растёт. Вторая: в&nbsp;жизни отражается среднее. Яркий день на&nbsp;семинаре сюда не&nbsp;засчитывается, провальный понедельник тоже. Считается ровный фон, который звучит в&nbsp;тебе изо дня в&nbsp;день. Поэтому разовый подъём выветривается за&nbsp;неделю, а&nbsp;поднятая планка держится и&nbsp;тянет за&nbsp;собой остальное.</p>
...
<h2>Почему на&nbsp;собеседовании спрашивают про&nbsp;близких</h2>
<p>Метод начинается с&nbsp;состояния. А&nbsp;сильнее всего на&nbsp;него влияет тот, кто рядом каждый день.</p>
<p>Шестнадцать лет практики сложились в&nbsp;простое наблюдение, и&nbsp;оно подтверждается на&nbsp;любой группе: пока дома годами идёт тихое перетягивание, туда и&nbsp;уходит основное, а&nbsp;работе достаётся остаток. Поэтому близкие отношения мы&nbsp;считаем рабочим вопросом, в&nbsp;одном ряду с&nbsp;делом и&nbsp;достатком, и&nbsp;на&nbsp;первом&nbsp;же разговоре уделяем им&nbsp;столько&nbsp;же времени, сколько планам. Что из&nbsp;этого выходит у&nbsp;мужчин с&nbsp;бизнесом, показано на&nbsp;<a href="/chizhovy2/dlya-predprinimatelej/">странице для предпринимателей</a>.</p>

<h2>Почему это делают в&nbsp;живой сцене</h2>
<p>Есть свойство памяти, которое объясняет всю механику. Когда воспоминание поднимают и&nbsp;проживают заново, оно ненадолго становится подвижным, и&nbsp;в&nbsp;этот момент к&nbsp;нему можно добавить другой опыт. Потом оно закрывается уже с&nbsp;поправкой.</p>
<p>Это описано в&nbsp;работах по&nbsp;памяти: Джозеф ЛеДу и&nbsp;Карим Надер показали сам эффект в&nbsp;2000&nbsp;году, позже Даниэла Шиллер проверила, что новый опыт в&nbsp;этом окне работает и&nbsp;без всяких препаратов. Окно короткое, счёт идёт на&nbsp;часы.</p>
<p>Поэтому в&nbsp;зале не&nbsp;пересказывают прошлое словами. Сцену поднимают целиком, с&nbsp;телом и&nbsp;эмоцией, и&nbsp;решение переписывают там&nbsp;же, пока окно открыто.</p>
...
<figure class="fig"><div class="ph"><img src="/chizhovy2/images/m1-krug.jpg" alt="Группа в кругу на первом модуле" loading="lazy" width="1360" height="768"></div><figcaption>Круг, в&nbsp;котором за&nbsp;два дня узнают друг о&nbsp;друге больше, чем соседи за&nbsp;годы</figcaption></figure>
...
<section><div class="narrow">
<figure class="fig"><div class="ph"><img src="/chizhovy2/images/m1-stulya.jpg" alt="Пустой круг стульев после работы" loading="lazy" width="1360" height="768"></div><figcaption>Зал перед началом. Через час здесь будет чужая история, в&nbsp;которой узнаешь свою</figcaption></figure>
...
<details><summary>Можно приехать с&nbsp;партнёром?</summary><p>Можно, и&nbsp;многие приезжают. Работать будете порознь, каждый со&nbsp;своей стороной. Для&nbsp;пар есть <a href="/chizhovy2/para/">отдельные условия по&nbsp;оплате</a>, о&nbsp;них говорим на&nbsp;собеседовании.</p></details>
</div>
<aside class="side">
...
<p>Мы&nbsp;предупреждаем об&nbsp;этом заранее, и&nbsp;сначала никто не&nbsp;верит. У&nbsp;тех, кто внутренне упирается, телефон начинает звонить даже выключенным: кажется, что вибрирует карман, что кто-то написал, что дома точно что-то случилось. Это не&nbsp;связь, это сопротивление ищет повод выйти из&nbsp;зала.</p>
</div>
</div>
</div></section>
...
<h2 style="margin-top:34px">Самое сильное лежит там, куда смотреть&nbsp;не&nbsp;хотелось</h2>
<p>Пять дней уходят на&nbsp;знакомство с&nbsp;теми частями себя, которые человек долго держал в&nbsp;стороне: злость, слабость, зависть, неудобное желание. Юнг называл это тенью. Спрятанное никуда не&nbsp;исчезает, оно просто выходит из&nbsp;поля зрения и&nbsp;дальше решает за&nbsp;человека из&nbsp;темноты: подбирает слова в&nbsp;ссоре, отговаривает от&nbsp;крупного шага. А&nbsp;ему кажется, что это был его собственный трезвый выбор.</p>
<p>Про принятие тени пишут на&nbsp;каждом углу. Здесь до&nbsp;неё доходят лично, при группе, в&nbsp;живой сцене. И&nbsp;почти всегда случается один и&nbsp;тот&nbsp;же поворот: опора обнаруживается ровно там, где её&nbsp;никто не&nbsp;искал. То, что казалось стыдным и&nbsp;слабым, оказывается источником силы. Оттуда потом и&nbsp;приходят перемены, на&nbsp;которые раньше не&nbsp;хватало ни&nbsp;решимости, ни&nbsp;энергии.</p>

<div class="card white" style="margin:26px 0">
<h3>Груз, которого не&nbsp;видно</h3>
<p>Алексей объясняет название модуля так. Представь: к&nbsp;тебе пристёгнута гиря на&nbsp;тридцать два килограмма. С&nbsp;ней ты&nbsp;бежишь марафон, качаешься, учишься работать эффективнее. Внутренняя свобода это просто найти её&nbsp;и&nbsp;отстегнуть.</p>
<p>Пока человек отводит взгляд, груз висит на&nbsp;каждом шаге. Посмотрел, признал: «да, это тоже я», и&nbsp;тяжести больше нет. Появляются силы, которых раньше будто не&nbsp;было. Все эти годы они уходили на&nbsp;то, чтобы держать гирю пристёгнутой.</p>
</div>
...
<section><div class="narrow">
<h2>Пока объясняешь, не&nbsp;проживаешь</h2>
<p>В&nbsp;зале это слышно раньше всего по&nbsp;речи. Человек выходит со&nbsp;своей историей и&nbsp;вместо неё выдаёт справку: время было тяжёлое, отец сам рос в&nbsp;таких&nbsp;же условиях, я&nbsp;тогда тянул две работы, она была не&nbsp;в&nbsp;себе. Сказанное чистая правда, каждое слово. И&nbsp;всё оно ради того, чтобы туда не&nbsp;возвращаться.</p>
<p>Объяснение это вежливая форма бегства. Оно снимает напряжение ровно настолько, чтобы жить дальше и&nbsp;внутрь не&nbsp;заглядывать. Пока идёт рассказ о&nbsp;причинах, сам случай стоит нетронутым: тело помнит его так&nbsp;же, как в&nbsp;первый день, а&nbsp;сверху лежит аккуратная версия для гостей и&nbsp;переговоров. За&nbsp;двадцать лет её&nbsp;отполировали до&nbsp;блеска.</p>
<p>Поэтому на&nbsp;модуле объяснения долго не&nbsp;живут: половина зала носит свои такие&nbsp;же и&nbsp;узнаёт их&nbsp;с&nbsp;полуслова. Разворот случается на&nbsp;короткой фразе, у&nbsp;которой нет продолжения: да, со&nbsp;мной это было. Без «потому что» и&nbsp;без имени виноватого. Признанное можно наконец прожить. И&nbsp;оправдывать его дальше уже незачем.</p>
</div></section>



<section><div class="narrow">
<h2>Опыт вместо объяснения</h2>
<p>О&nbsp;том, как устроена реальность, на&nbsp;модуле не&nbsp;читают лекций. Пять дней собраны как пространство, где человек доходит до&nbsp;этого сам: видит, как из&nbsp;его состояния складывается то, что с&nbsp;ним происходит, и&nbsp;почему одни и&nbsp;те&nbsp;же истории возвращаются к&nbsp;нему с&nbsp;разными лицами.</p>
<p>Сказать это словами можно за&nbsp;десять минут, и&nbsp;через день в&nbsp;голове будет одна красивая формулировка, которую приятно повторять в&nbsp;компании. Прожитое остаётся с&nbsp;человеком: тело помнит, как это было, и&nbsp;спорить с&nbsp;ним уже не&nbsp;получается.</p>
</div></section>
...
<h2>Про кого этот список на&nbsp;самом деле</h2>
<p>У&nbsp;пар это вскрывается почти всякий раз. То, что человек годами предъявляет партнёру (холодный, равнодушный, вечно занят), в&nbsp;работе поворачивается на&nbsp;него самого. Та&nbsp;же черта живёт и&nbsp;в&nbsp;нём, просто зовётся иначе и&nbsp;всегда объяснима. Психологи называют это проекцией, в&nbsp;зале её&nbsp;видно и&nbsp;без термина.</p>
<p>Открытие обычно приносит облегчение. Оно снимает с&nbsp;партнёра роль главного препятствия и&nbsp;возвращает человеку собственный ход: дальше в&nbsp;сцене работают уже с&nbsp;этим.</p>
<p>Самый частый страх пар звучит так: при нём я&nbsp;не&nbsp;раскроюсь. Здесь есть простое решение, и&nbsp;оно работает уже много лет. Если одному тяжело говорить при своём человеке, того просят подождать за&nbsp;дверью, и&nbsp;он проживает своё сам. Никто не&nbsp;заставляет вскрываться на&nbsp;глазах у&nbsp;того, перед кем сейчас страшно.</p>
...
<h2>Ирина и&nbsp;Алексей</h2>
<div class="grid2" style="margin-top:24px">
<div class="card white">{icon('flame','var(--sand)')}<h3>Ирина</h3><p>Трансформационный и&nbsp;танцевальный тренер, больше 25&nbsp;лет работы с&nbsp;людьми. Самые глубокие программы погружения вела одна. Шесть лет готовилась к&nbsp;этому формату под руководством наставника. Участники говорят, что она «вскрывает и&nbsp;собирает», и&nbsp;вспоминают её работу&nbsp;годами.</p></div>
<div class="card white">{icon('mountain')}<h3>Алексей</h3><p>Сертифицированный коуч, 18&nbsp;лет практики. Держит структуру и&nbsp;точность процесса: с&nbsp;ним безопасно идти в&nbsp;глубину, потому что он&nbsp;видит дорогу&nbsp;целиком.</p></div>
</div>
...
<p>Дальше пошла практика: сотни залов, тысячи разобранных сцен, свои ошибки ведущих и&nbsp;свои находки. Из&nbsp;четырёх источников осталось то, что реально меняет жизнь участников, остальное отсеялось. Как именно отбирали, разобрано в&nbsp;<a href="/chizhovy2/istoki/" style="color:#D08A5F">истоках метода</a>.</p>
...
<h2 style="margin-top:34px">Свою тему разбирают при&nbsp;группе</h2>
<p>Иногда на&nbsp;группе всплывает тема, которая задевает самих ведущих. Тогда они разбирают её&nbsp;здесь&nbsp;же, при всех, наравне с&nbsp;участниками: называют, что зацепило, и&nbsp;идут в&nbsp;это до&nbsp;конца. Для группы это часть метода: видно живьём, что процесс не&nbsp;заканчивается. Ни&nbsp;на&nbsp;семнадцатом году вместе, ни&nbsp;на&nbsp;сотом зале.</p>
</div></section>
...
<details><summary>Какие правила действуют в&nbsp;зале?</summary><p>Семь соглашений, их&nbsp;называют вслух в&nbsp;первый вечер: конфиденциальность, быть вовремя, выключенные телефоны, обращение на&nbsp;«ты», трезвость, правило «стоп» и&nbsp;честность к&nbsp;себе. Каждое разобрано на&nbsp;<a href="/chizhovy2/bezopasnost/">странице про безопасность</a>.</p></details>
<details><summary>Кому сейчас точно не&nbsp;сюда?</summary><p>Тем, кому нужна медицинская помощь: в&nbsp;остром состоянии погружение скорее нагрузит. Тем, у&nbsp;кого прямо сейчас переезд, роды или аврал: работа требует сил. И&nbsp;тем, кто ищет готовый скрипт, как заставить измениться другого. Об&nbsp;этом мы&nbsp;говорим прямо на&nbsp;собеседовании.</p></details>
</div>
<aside class="side">
...
def splav_svg():
    """Истоки-обзор: источники сходятся в знак школы и уходят в зал.
    Координаты считаются от числа источников, чтобы схема не ломалась при правке списка."""
    имена = [("Морено","сцена"),("Зеланд","язык"),("Годдард","состояние"),("Наука","проверка")]
    шаг = 720 / (len(имена) - 1)
...
<h2>Четыре источника, один метод</h2>
...
<div class="chiplist" style="text-align:center"><span>Морено · сцена</span><span>Зеланд · язык</span><span>Годдард · состояние</span><span>Наука · проверка</span></div>
...
<section><div class="wrap"><div class="tside">
<div class="col">
<p class="eyebrow">Как четыре опоры сходятся&nbsp;вместе</p>
...
<p>Рамка вокруг всего этого пришла из&nbsp;жанра погружений: несколько дней подряд, ясные правила, длинный день, из&nbsp;которого не&nbsp;выпрыгнуть в&nbsp;привычные дела и&nbsp;звонки. За&nbsp;два часа встречи человек не&nbsp;успевает выйти из-под своей&nbsp;защиты.</p>
<p>Вечером разбор на&nbsp;языке, который прижился в&nbsp;группах: где сегодня зацепило, где снял важность, что заметил за&nbsp;собой. Здесь удобен словарь Зеланда. А&nbsp;под всем этим лежит наука: полторы минуты паузы после сильного момента, названное вслух чувство, точная формулировка намерения. Четыре источника не&nbsp;лежат по&nbsp;отдельным полкам, человек проходит через все за&nbsp;один&nbsp;день.</p>
</div>
<aside class="side">
...
<p>Не&nbsp;обязательно. Книга Зеланда стоит в&nbsp;списке чтения в&nbsp;Игре пробуждения, остальное происходит уже&nbsp;в&nbsp;зале.</p>
...
{istoki_dalee(("/chizhovy2/istoki/zeland-transerfing/", "Зеланд и трансерфинг"), ("/chizhovy2/istoki/goddard/", "Невилл Годдард"), ("/chizhovy2/metod/", "Метод целиком"))}
<section><div class="wrap"><div class="tside">
<div class="col">
...
{istoki_dalee(("/chizhovy2/istoki/goddard/", "Невилл Годдард"), ("/chizhovy2/istoki/moreno-psihodrama/", "Морено и психодрама"), ("/chizhovy2/metod/", "Метод целиком"))}
<section><div class="wrap"><div class="tside">
<div class="col">
...
<p>Формулировка, которая на&nbsp;разборах звучит чаще всего: делаю для всех, а&nbsp;в&nbsp;ответ вот это. Сказано честно, и&nbsp;выхода из&nbsp;этой точки не&nbsp;видно. Сдвиг начинается в&nbsp;тот момент, когда мужчина смотрит на&nbsp;ту&nbsp;же историю с&nbsp;другой стороны и&nbsp;спрашивает, где он&nbsp;собрал её&nbsp;своими руками. Жертва и&nbsp;автор разобраны на&nbsp;<a href="/chizhovy2/metod/">странице метода</a>.</p>
...
<p>К&nbsp;вечеру приходит настоящая усталость. Заниматься собственной жизнью столько часов подряд непривычно, и&nbsp;тело чувствует это раньше головы. После зала команда чаще всего идёт ужинать вместе. До&nbsp;двадцати двух каждый пишет в&nbsp;чат открытия и&nbsp;благодарности. Смысл в&nbsp;том, чтобы назвать увиденное словами в тот&nbsp;же вечер: иначе к&nbsp;утру от&nbsp;него остаётся только общее хорошее&nbsp;чувство. Мы&nbsp;и&nbsp;сами это советуем: после тяжёлого дня лучше не&nbsp;разъезжаться сразу. Посидеть вместе, поужинать, помолчать рядом. То, что открылось днём, укладывается быстрее среди тех, кто был там&nbsp;же.</p>
</div>
<aside class="side">
...
<div class="card white" style="margin-bottom:12px">{icon('route','var(--copper)')}<h3>Можно выйти и&nbsp;сказать «я&nbsp;не&nbsp;готов»</h3><p>Бывает, человек выходит и&nbsp;понимает, что сейчас не&nbsp;может. Тогда он&nbsp;говорит об&nbsp;этом вслух, садится обратно и&nbsp;остаётся в&nbsp;зале. Из&nbsp;группы за&nbsp;это не&nbsp;выводят и&nbsp;к&nbsp;сцене не&nbsp;возвращают силой: работа начинается тогда, когда человек сам готов её&nbsp;начать.</p></div>
...
<p>На&nbsp;Новый год выпускники съезжаются из&nbsp;разных городов. Уже без ведущих и&nbsp;без программы, просто потому что за&nbsp;три месяца люди стали друг другу ближе, чем многие, кого они знают двадцать лет.</p>
...
<div class="grid2" style="margin-top:22px">
<div class="card"><h3>Откуда всё&nbsp;это</h3><p><a href="/chizhovy2/istoki/">Четыре источника метода</a> названы открыто, вместе с&nbsp;наукой, на&nbsp;которую они опираются.</p></div>
...
<aside class="side">
<div class="box"><div class="lbl">Открыто</div><div class="big">4 опоры</div><p>Морено и&nbsp;психодрама, трансерфинг, Годдард и&nbsp;наука под каждым инструментом.</p></div>
</aside>
</div></div></section>
...
<div class="grid2" style="margin-top:22px">
<div class="card"><h3>Заполняешь&nbsp;анкету</h3><p>Несколько вопросов: что происходит сейчас, что повторяется, что уже пробовал и к&nbsp;чему хочешь прийти. Занимает 15-20&nbsp;минут, <a href="https://forms.yandex.ru/cloud/684dcab0f47e730799e7cb6d" target="_blank" rel="noopener">ссылка на&nbsp;анкету</a>. В&nbsp;ответ придёт ещё и&nbsp;разбор <a href="/chizhovy2/metod/">метода школы</a>, чтобы к&nbsp;разговору ты&nbsp;уже понимал, как здесь работают.</p></div>
...
<div class="card"><h3>Говорим&nbsp;голосом</h3><p>Разговор ведёт кто-то из&nbsp;школы, человек, который сам работает в&nbsp;зале. Как он&nbsp;идёт, расписано на&nbsp;странице&nbsp;<a href="/chizhovy2/sessiya/">собеседования</a>.</p></div>
...
<div class="st">{icon('loop')}<div><b>Трансерфинг Зеланда</b>
<div class="chiplist"><span>Маятники простыми словами</span><span>Важность и&nbsp;как её&nbsp;снять</span><span>Намерение против желания</span></div></div></div>
...
<div class="st">{icon('mountain','var(--sand)')}<div><b>Истоки&nbsp;метода</b><p>Четыре источника с&nbsp;разбором каждого: психодрама Морено, трансерфинг, Годдард и&nbsp;наука.&nbsp;<a href="/chizhovy2/istoki/">Смотреть истоки</a></p></div></div>
</div>
</div></section>
